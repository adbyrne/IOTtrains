...
| 2.9 | 2026-06-17 | Block signals converted to momentary (per owner direction): signal normally `lowered` (stop); a dispatcher click triggers a `BLOCK_SIGNAL_PULSE_SECONDS` (60 real-second) clear pulse that auto-reverts server-side — no manual "leave it raised" path. A click while a pulse is already active is rejected (409), not extended or aborted. `POST /api/signal/block` body simplified to `{station_id}` (no `state` — trigger-only). `state.block_signals` now initializes both WP/XP to `lowered` at startup instead of being unset. MQTT cmd/state payload schema unchanged — hardware-agnostic, all timing lives server-side. MQTT_SPEC.md §2.6a updated. 4 new/updated tests, 155 total passing. |
