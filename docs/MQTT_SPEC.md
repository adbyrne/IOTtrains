...
TO text is generated by the dispatcher UI from a **structured template** — the dispatcher selects the TO type and fills in the required fields; the system formats the text. TO types, field definitions, and text templates are defined in `data/to_types.json`. The formatted text is what appears in `text` below and on the printed form the dispatcher copies to their paper log.
...
| `form` | string | TO form — stamped from `data/to_types.json` `default_form`; `"19"` (train must stop) or `"31"` (proceed). NY&E always `"19"`. |
...
| `text` | string | Full formatted order text (generated from structured template by dispatcher) |
