...
| `extra` | bool | True if extra train (not in timetable); engine number used as train identifier |
| `work_extra` | bool | True if work extra (implies `extra: true`); work extras have no fixed schedule |
...
The dispatcher selects the TO type and fills in the required fields. **No pre-rendered text is included in the payload** — each receiver (dispatcher UI, CYD firmware) renders the order text locally from the type's template in `data/to_types.json`. This keeps the payload compact and lets each display format the text appropriately.

**Valid `to_type` values:** `meet`, `wait`, `running_extra`, `work_extra`, `annulment`, `sections` — see `data/to_types.json` for full field schemas and templates.

**`addressed_to` selection:** The dispatcher manually selects which stations receive each order. For extras, `trains` contains the engine number; for scheduled trains, `trains` contains the timetable train number.

**Signal arm / TO workflow:** Before issuing a TO, the dispatcher raises the appropriate TO signal arms directly via the dispatcher UI (separate action from TO issuance). This stops trains at the relevant stations. After all addressed stations ACK, the dispatcher manually lowers the arms.
...
| `form` | string | TO form number, stamped from `data/to_types.json` `default_form`. `"19"` = standard order, can be hooped to moving train. `"31"` = stop-and-sign order, crew must stop and sign receipt, requires clearance. NY&E always uses `"19"`. |
| `to_type` | string | Order type — one of `"meet"`, `"wait"`, `"running_extra"`, `"work_extra"`, `"annulment"`, `"sections"` |
| `trains` | array | Train/engine numbers this order applies to (used by CYD to match TO to OS submission) |
| `addressed_to` | array | All station IDs receiving this order |
| `issued_rr_time` | string | Railroad time of issuance |
| `day` | int | Session day |
| `fields` | object | Type-specific field values (see `data/to_types.json` for schema per type). Each receiver renders order text from these fields using the type's template. |
...
#### `trains/yard/consist/{train_number}`
**Direction:** Yardmaster terminal → All  
**QoS:** 1 | **Retained:** Yes
...
```json
{
  ...
  "cars_empty": 5,
  "cars_total": 17,
  "cleared_rr_time": "15:52",
  "day": 1
}
```

Engine and caboose are retained in the cleared payload so the dispatcher can confirm identity against incoming OS reports after the train departs.
...
| `cars_loaded` | int | Loaded freight car count |
| `cars_empty` | int | Empty freight car count |
| `cars_total` | int | Derived: loaded + empty |
| `rr_time` | string | Railroad time of submission (assembling/ready) |
| `cleared_rr_time` | string | Railroad time of clearance issuance (cleared only) |
| `day` | int | Session day |
...
### 2.11 Extra Train Schedule

#### `trains/extra/{engine}/schedule`
**Direction:** RR_Server → All  
**QoS:** 1 | **Retained:** Yes  
**`{engine}`:** Engine number (extra train identity)

Published by RR_Server when the dispatcher creates a running extra via the structured TO form. Retained so the yardmaster terminal and any reconnecting client can recover active extras.
...
```json
{
  ...
  "direction": "N",
  "extra_class": 3,
  "departure_station": "WP",
  "departure_rr_time": "06:00",
  "day": 1,
  "schedule": [
    { "location": "WP",  "depart": "06:00" },
    { "location": "XP",  "arrive": "10:09" },
    { "location": "BB",  "arrive": "10:46" },
    { "location": "JC",  "arrive": "11:06" },
    { "location": "SK",  "arrive": "11:25" },
    { "location": "HC",  "arrive": "11:44" }
  ],
  "to_seq": 12
}
```

`schedule` times are calculated from segment travel times (timetable `segments` data). `to_seq` is the sequence number of the authorizing TO for cross-reference. Work extras do not use this topic — they are described in the authorizing TO text only.
...
| 0.2 | 2026-04-29 | TO signal topics; yard messages; block detection namespace |
| 0.3 | 2026-04-30 | QoS assignments; retained flags; LWT config; camera and block stubs |
| 0.4 | 2026-05-05 | TOs: structured templates, added `to_type` field. Consist: per-train retained topic `trains/yard/consist/{train_number}`; `state` field (assembling/ready/cleared); `extra` flag; `cars_total`; cleared payload. Notification: `cancellation` → `annulment`. New section 2.11: `trains/extra/{engine}/schedule` retained topic. |
